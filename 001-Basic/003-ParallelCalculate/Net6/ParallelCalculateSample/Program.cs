...
namespace ParallelCalculateSample
{
    class Program
    {
        static async Task Main(string[] args)
        {
            ...
            await ProcessByParallelAsync(testData, 8);
            await ProcessByParallelAsync(testData, 4);
            await ProcessByParallelAsync(testData, 2);
            await ProcessByParallelAsync(testData, 1);
        ...
        /// <summary>
        /// 使用 Parallel.ForEachAsync (.NET 6 新增的函式)
        /// </summary>
        /// <param name="data"></param>
        /// <param name="threads"></param>
        /// <returns></returns>
        static async Task<List<string>> ProcessByParallelAsync(List<string> data, int threads = 0) 
        {
            DateTime start = DateTime.Now;
            ...
            var results = new ConcurrentStack<string>();
            await Parallel.ForEachAsync(data, options, async (item, loopState) => {
            ...
            Console.WriteLine($"Parallel.ForEachAsync ({threads} Threads): {results.Count} Records, { (end - start).TotalMilliseconds } ms");
